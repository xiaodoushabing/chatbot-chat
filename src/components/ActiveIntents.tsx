    ...
    <div className="flex flex-col gap-8 p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h2 className="text-4xl font-bold tracking-tight text-slate-900">Active Intent Database</h2>
          <p className="text-slate-500 text-base">Manage and monitor all live chatbot intents and their performance.</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="px-6 py-4 text-sm font-bold text-slate-500 uppercase tracking-widest">Intent Name</th>
              ...
              <th className="px-6 py-4 text-sm font-bold text-slate-500 uppercase tracking-widest text-center">Queries</th>
              <th className="px-6 py-4 text-sm font-bold text-slate-500 uppercase tracking-widest">Last Updated</th>
              ...
              >
                <td className="px-6 py-5">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-slate-900 text-base">{intent.name}</span>
                  </div>
                </td>
                <td className="px-6 py-5">
                ...
                <td className="px-6 py-5">
                  <div className="flex items-center gap-2">
                    <div className={cn("w-2 h-2 rounded-full", intent.status === 'active' ? "bg-emerald-500" : "bg-slate-400")} />
                    <span className={cn("text-sm font-bold uppercase tracking-wider", intent.status === 'active' ? "text-emerald-700" : "text-slate-500")}>
                      {intent.status}
                    </span>
                  </div>
                </td>
                <td className="px-6 py-5 text-center">
                  <span className="text-base font-mono text-slate-600">{intent.queries.toLocaleString()}</span>
                </td>
                <td className="px-6 py-5">
                  <div className="flex items-center gap-2 text-slate-500">
                    <Clock size={16} />
                    <span className="text-sm">{intent.lastUpdated}</span>
                  </div>
                </td>
                <td className="px-6 py-5 text-right">
                  <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    >
                      {intent.status === 'active' ? <PowerOff size={18} /> : <Power size={18} />}
                    </button>
                    <button
      ...
      <div className="flex items-center justify-between px-2">
        <span className="text-base text-slate-500">Showing {intents.length} of {intents.length} intents</span>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                <h3 className="text-2xl font-bold text-slate-900">Edit Intent</h3>
                <button
                  onClick={() => setEditingIntent(null)}
              ...
              <form onSubmit={handleSaveEdit} className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
                <div className="flex flex-col gap-2">
                  <label className="text-base font-bold text-slate-700">Intent Name</label>
                  <input
                    type="text"
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <label className="text-base font-bold text-slate-700">Utterances ({editingIntent.utterances.length})</label>
                    <button
                      type="button"
                      onClick={handleAddUtterance}
                      className="text-sm font-bold text-[#E3000F] hover:text-red-700 flex items-center gap-1"
                    >
                      <Plus size={16} /> Add
                    </button>
                  </div>
                    ...
                    {editingIntent.utterances.map((utt, i) => (
                      <div key={i} className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-base outline-none focus:ring-2 focus:ring-[#E3000F]"
                        />
                        <button
                          type="button"
                ...
                {/* Response */}
                <div className="flex flex-col gap-2">
                  <label className="text-base font-bold text-slate-700">Response</label>
                  <textarea
                    value={editingIntent.response}
                    onChange={(e) => setEditingIntent({ ...editingIntent, response: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-base outline-none focus:ring-2 focus:ring-[#E3000F] min-h-[120px] resize-y"
                    required
                  />
                </div>
